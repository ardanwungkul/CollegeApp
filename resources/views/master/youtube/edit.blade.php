<x-app-layout>
    <div class="mx-auto max-w-3xl space-y-3">
        <p class="text-white text-lg font-bold">Edit Youtube</p>
                ...
                <div class="space-y-3">
                    <div class="flex flex-col gap-1">
                        ...
                        <input type="url" id="link" name="link"
                            value="https://www.youtube.com/watch?v={{ $youtube->link }}"
                        ...
                        <select name="user[]" id="user" required
                            ...
                            @foreach ($user as $item)
                                <option value="{{ $item->id }}"
                                    {{ in_array($item->id, $youtube->user->pluck('id')->toArray()) ? 'selected' : '' }}
                                    data-group="{{ $item->group->pluck('id') }}">
                                    {{ $item->name }}
                                </option>
                            @endforeach
                        </select>
